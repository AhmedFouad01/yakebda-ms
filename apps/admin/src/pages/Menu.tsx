...
const YK_SAUCE = {
  name_ar: "الصوص والشطة",
  min_select: 0,
  max_select: 3,
  is_required: false,
  modifiers: [
    { name_ar: "شطة", price_delta: 0 },
    { name_ar: "بدون شطة", price_delta: 0 },
    { name_ar: "طحينة", price_delta: 2 },
    { name_ar: "دقة", price_delta: 2 },
  ],
};
const YK_EXTRAS = {
  name_ar: "إضافات يا كبدة",
  min_select: 0,
  max_select: 5,
  is_required: false,
  modifiers: [
    { name_ar: "جبنة", price_delta: 8 },
    { name_ar: "مخلل زيادة", price_delta: 3 },
    { name_ar: "عيش زيادة", price_delta: 3 },
    { name_ar: "بطاطس جوه الساندوتش", price_delta: 6 },
  ],
};
const YK_SIZES = [
  { name_ar: "لقمة صغير", price_delta: 0 },
  { name_ar: "همر كبير", price_delta: 15 },
];
const YAKEBDA_TEMPLATE = [
  { category: "ساندوتشات", name_ar: "ساندوتش كبدة إسكندراني", sku: "SAN-KBD-ISK", base_price: 30, ingredients_ar: "كبدة، فلفل، ثوم، ليمون، خلطة يا كبدة", portion_note_ar: "لقمة صغير / همر كبير", variants: YK_SIZES, modifier_groups: [YK_SAUCE, YK_EXTRAS] },
  { category: "ساندوتشات", name_ar: "ساندوتش كبدة بلدي", sku: "SAN-KBD-BLD", base_price: 32, ingredients_ar: "كبدة بلدي، بصل، فلفل، توابل", portion_note_ar: "لقمة صغير / همر كبير", variants: YK_SIZES, modifier_groups: [YK_SAUCE, YK_EXTRAS] },
  { category: "ساندوتشات", name_ar: "ساندوتش سجق", sku: "SAN-SGG", base_price: 30, ingredients_ar: "سجق، صلصة، فلفل، توابل", portion_note_ar: "لقمة صغير / همر كبير", variants: YK_SIZES, modifier_groups: [YK_SAUCE, YK_EXTRAS] },
  { category: "ساندوتشات", name_ar: "ساندوتش كبدة وسجق ميكس", sku: "SAN-MIX", base_price: 38, ingredients_ar: "كبدة، سجق، فلفل، صوص", portion_note_ar: "لقمة صغير / همر كبير", variants: YK_SIZES, modifier_groups: [YK_SAUCE, YK_EXTRAS] },
  { category: "ساندوتشات", name_ar: "ساندوتش كبدة بالجبنة", sku: "SAN-KBD-CHS", base_price: 40, ingredients_ar: "كبدة، جبنة، فلفل، خلطة", portion_note_ar: "لقمة صغير / همر كبير", variants: YK_SIZES, modifier_groups: [YK_SAUCE, YK_EXTRAS] },
  { category: "أطباق", name_ar: "طبق كبدة ربع كيلو", sku: "PLT-KBD-250", base_price: 95, ingredients_ar: "ربع كيلو كبدة، عيش، مخلل، صوص", portion_note_ar: "ربع كيلو" },
  { category: "أطباق", name_ar: "طبق كبدة نص كيلو", sku: "PLT-KBD-500", base_price: 175, ingredients_ar: "نص كيلو كبدة، عيش، مخلل، صوص", portion_note_ar: "نص كيلو" },
  { category: "أطباق", name_ar: "طبق سجق ربع كيلو", sku: "PLT-SGG-250", base_price: 90, ingredients_ar: "ربع كيلو سجق، عيش، مخلل، صوص", portion_note_ar: "ربع كيلو" },
  { category: "وجبات", name_ar: "وجبة كبدة لقمة", sku: "MEAL-KBD-L", base_price: 65, ingredients_ar: "ساندوتش كبدة، بطاطس، مشروب", portion_note_ar: "كومبو فردي" },
  { category: "وجبات", name_ar: "وجبة همر كبدة", sku: "MEAL-KBD-H", base_price: 85, ingredients_ar: "ساندوتش همر كبدة، بطاطس، مشروب", portion_note_ar: "كومبو كبير" },
  { category: "إضافات", name_ar: "بطاطس", sku: "EXT-FRIES", base_price: 20, ingredients_ar: "بطاطس مقلية", portion_note_ar: "علبة" },
  { category: "إضافات", name_ar: "عيش زيادة", sku: "EXT-BREAD", base_price: 5, ingredients_ar: "عيش", portion_note_ar: "قطعة" },
  { category: "إضافات", name_ar: "مخلل", sku: "EXT-PICKLES", base_price: 7, ingredients_ar: "مخلل مشكل", portion_note_ar: "علبة صغيرة" },
  { category: "مشروبات", name_ar: "بيبسي", sku: "DRK-PEPSI", base_price: 15, ingredients_ar: "مشروب غازي", portion_note_ar: "كانز" },
  { category: "مشروبات", name_ar: "مياه", sku: "DRK-WATER", base_price: 8, ingredients_ar: "مياه معدنية", portion_note_ar: "زجاجة" },
];
...
function CategoriesTab() {
  ...
  return (
    <>
      ...
      <div className="panel"><table><thead><tr><th>{t.menu.nameAr}</th><th>{t.menu.sortOrder}</th><th></th></tr></thead><tbody>
        {data.map((c) => <tr key={c.id}><td>{c.name_ar}</td><td>{c.sort_order}</td><td><button onClick={() => toggle(c)}>{c.is_active ? t.common.active : t.common.inactive}</button></td></tr>)}
      </tbody></table></div>
    </>
  );
}
...
function ProductsTab() {
  ...
  const [vName, setVName] = useState("لقمة صغير");
    ...
    try {
      const created = await api<{ data: Product }>("/products", {
        ...
        body: {
          ...
          portion_note_ar: form.portion_note_ar || null,
          description_ar: form.description_ar || null,
          base_price: Number(form.base_price),
          sort_order: data.length,
        },
      });
      if (form.category_id && cats.data.find((c) => c.id === form.category_id)?.name_ar === "ساندوتشات") {
        await api(`/products/${created.data.id}/variants`, { method: "POST", body: { name_ar: "لقمة صغير", price_delta: 0 } });
        await api(`/products/${created.data.id}/variants`, { method: "POST", body: { name_ar: "همر كبير", price_delta: 15 } });
      }
  ...
  async function addVariant(p: Product) {
    try { await api(`/products/${p.id}/variants`, { method: "POST", body: { name_ar: vName, price_delta: vDelta } }); setVName(""); setVDelta(0); setErr(""); reload(); }
    catch (e: any) { setErr(e.message); }
  }
  async function toggleGroup(p: Product, gid: string) {
    ...
    try { await api(`/products/${p.id}/modifier-groups`, { method: "PUT", body: { modifier_group_ids: next } }); setErr(""); reload(); }
    catch (e: any) { setErr(e.message); }
  }
  ...
  return (
    <>
      ...
      <div className="form-row menu-product-form">
        ...
        <input placeholder="الحجم/الوصف: لقمة صغير / همر كبير" value={form.portion_note_ar} onChange={(e) => setForm({ ...form, portion_note_ar: e.target.value })} />
        <button className="primary" onClick={add} disabled={!form.name_ar || !form.category_id}>إضافة صنف كامل</button>
      </div>
      <div className="panel">
        <table>
          <thead><tr><th>الصنف</th><th>الفئة</th><th>السعر</th><th>التفاصيل</th><th>الأحجام</th><th></th></tr></thead>
          <tbody>
            {data.map((p) => (
              <Fragment key={p.id}>
                <tr>
                  <td><b>{p.name_ar}</b><div className="muted">{p.sku || "بدون SKU"}</div></td>
                  <td>{cats.data.find((c) => c.id === p.category_id)?.name_ar}</td>
                  <td><input type="number" min={0} defaultValue={Number(p.base_price)} style={{ width: 90 }} onBlur={(e) => Number(e.target.value) !== Number(p.base_price) && patch(p.id, { base_price: Number(e.target.value) })} /></td>
                  <td><div>{p.ingredients_ar || "—"}</div><div className="muted">{p.portion_note_ar || "—"}</div></td>
                  <td>{p.variants.map((v) => v.name_ar).join("، ") || "—"}</td>
                  <td><button onClick={() => setExpanded(expanded === p.id ? "" : p.id)}>{expanded === p.id ? t.pos.close : t.orders.details}</button>{" "}<button onClick={() => patch(p.id, { is_active: !p.is_active })}>{p.is_active ? t.common.active : t.common.inactive}</button></td>
                </tr>
                {expanded === p.id && (
                  <tr>
                    <td colSpan={6}>
                      <div className="form-row">
                        <input placeholder="المكونات" defaultValue={p.ingredients_ar ?? ""} onBlur={(e) => patch(p.id, { ingredients_ar: e.target.value || null })} />
                        <input placeholder="الحجم/وصف الحصة" defaultValue={p.portion_note_ar ?? ""} onBlur={(e) => patch(p.id, { portion_note_ar: e.target.value || null })} />
                        <input placeholder="رابط الصورة" dir="ltr" defaultValue={p.image_url ?? ""} onBlur={(e) => patch(p.id, { image_url: e.target.value || null })} />
                      </div>
                      <div className="form-row">
                        <input placeholder="اسم الحجم" value={vName} onChange={(e) => setVName(e.target.value)} />
                      ...
                      <div className="mod-group-name">مجموعات الإضافات المرتبطة</div>
                      <div className="seg wrap">{groups.data.map((g) => <button key={g.id} className={p.modifier_group_ids.includes(g.id) ? "active" : ""} onClick={() => toggleGroup(p, g.id)}>{g.name_ar}</button>)}</div>
                    </td>
                  </tr>
                )}
              </Fragment>
...
function ImportTab() {
  ...
  async function runImport() {
    try {
      ...
      const res = await api<{ data: { created: number; updated: number; total: number } }>("/products/import", { method: "POST", body: { items } });
      setMsg(`تم الاستيراد: ${res.data.total} صنف — جديد ${res.data.created} / تحديث ${res.data.updated}`);
      setErr("");
    } catch (e: any) { setErr(e.message); }
  }
  return (
    <div className="import-grid">
      <div className="panel import-panel">
        ...
        <p className="muted">الصيغة JSON. تقدر تستورد صنف واحد أو مئات الأصناف مرة واحدة. لو الـ SKU موجود يتم تحديثه بدل تكراره.</p>
        {err && <div className="alert">{err}</div>}{msg && <div className="ok">{msg}</div>}
        <div className="form-row">
          <button className="primary" onClick={() => setText(JSON.stringify({ items: YAKEBDA_TEMPLATE }, null, 2))}>تحميل قالب يا كبدة</button>
