...
export function Pos() {
  ...
  const visible = useMemo(() => {
    ...
    if (search) return all.filter((p) => p.name_ar.includes(search) || p.ingredients_ar?.includes(search) || p.portion_note_ar?.includes(search));
    return categories.find((c) => c.id === activeCat)?.products ?? [];
  ...
  return (
    <div className="posx" dir="rtl">
      <header className="posx-head">
        <div className="posx-brand"><img src={brand.logoPath} alt={brand.nameAr} /><div><strong>{brand.nameAr}</strong><span>{brand.systemName}</span></div></div>
      ...
      <div className="posx-body">
        <section className="posx-menu">
          <div className="posx-cats">{categories.map((c) => <button key={c.id} className={c.id === activeCat && !search ? "active" : ""} onClick={() => { setActiveCat(c.id); setSearch(""); }}>{c.name_ar}</button>)}</div>
          <div className="posx-grid">
            {visible.map((p) => (
              <button key={p.id} className={p.is_available ? "posx-card" : "posx-card off"} onClick={() => pick(p)}>
                {settings?.show_product_images !== false && (p.image_url ? <img className="posx-card-img" src={p.image_url} alt={p.name_ar} /> : <span className="posx-card-img ph">{p.name_ar.trim().charAt(0)}</span>)}
                <span className="posx-card-name">{p.name_ar}</span>
                {p.portion_note_ar && <span className="posx-card-size">{p.portion_note_ar}</span>}
                <span className="posx-card-price">{money(p.effective_price)}</span>
                {p.ingredients_ar && <span className="posx-card-ing">{p.ingredients_ar}</span>}
        ...
        <aside className="posx-cart">
          ...
          <div className="posx-opts">
            ...
            <div className="seg dark wrap">{enabledMethods.map((k) => <button key={k} className={payment === k ? "active" : ""} onClick={() => setPayment(k)}>{t.pos[k as "cash" | "card" | "wallet" | "unpaid"]}</button>)}</div>
            {cashBlocked && <div className="posx-warn">{t.shift.cashNeedsShift}</div>}
          </div>

          <div className="posx-calc"><div className="posx-calc-display" dir="ltr">{calc || "0"}</div>{["7","8","9","÷","4","5","6","×","1","2","3","-","0",".","=","+","C","⌫","خصم","دليفري"].map((k) => <button key={k} onClick={() => calcPress(k)}>{k}</button>)}</div>
          ...
          <div className="posx-fire"><button disabled={!cart.length || busy} onClick={() => fireOrder({ submit: true, pay: false, print: false })}>{t.pos.kot}</button><button disabled={!cart.length || busy} onClick={() => fireOrder({ submit: false, pay: false, print: false })}>{t.pos.bill}</button><button className="hot" disabled={!cart.length || busy || payment === "unpaid" || cashBlocked} title={cashBlocked ? t.shift.cashNeedsShift : undefined} onClick={() => fireOrder({ submit: true, pay: true, print: false })}>{t.pos.billPay}</button>{settings?.receipt_printing_enabled && <button className="hot" disabled={!cart.length || busy || cashBlocked} title={cashBlocked ? t.shift.cashNeedsShift : undefined} onClick={() => fireOrder({ submit: true, pay: payment !== "unpaid", print: true })}>{t.pos.billPrint}</button>}<button className="ghost" disabled={!cart.length} onClick={() => setCart([])}>{t.pos.clear}</button></div>
        </aside>
      </div>
