...
import { money, paymentLabels, unitPrice } from "./posUtils";
    ...
    <>
      <aside id="posx-cart-drawer" className={`posx-cart${cartDrawerOpen ? " is-open" : ""}`}>
        ...
        <div className="posx-cart-scroll">
          <div className="posx-cart-lines">
            ...
            {cart.map((line, index) => (
              <PosCartLine
                ...
                onIncrease={() => setCart((rows) => rows.map((row, itemIndex) => itemIndex === index ? { ...row, qty: row.qty + 1 } : row))}
                onDecrease={() => setCart((rows) => rows.flatMap((row, itemIndex) => itemIndex !== index ? [row] : row.qty > 1 ? [{ ...row, qty: row.qty - 1 }] : []))}
                onRemove={() => setCart((rows) => rows.filter((_, itemIndex) => itemIndex !== index))}
                onNotesChange={(notes) => setCart((rows) => rows.map((row, itemIndex) => itemIndex === index ? { ...row, notes } : row))}
              />
            ))}
          </div>
          ...
          <div className="posx-opts">
            ...
            {settings?.allow_discounts !== false && (
              <>
                <input type="number" min={0} placeholder={t.pos.discount} value={discount || ""} onChange={(event) => setDiscount(Number(event.target.value))} />
                {discount > 0 && settings?.discount_reason_required && (
                  <input placeholder={t.pos.discountReason} value={discountReason} onChange={(event) => setDiscountReason(event.target.value)} />
                )}
        ...
        <div className="posx-fire-wrap">
          <button className="posx-order-now" disabled={busy || !!payReason} title={payReason ?? undefined} onClick={fireOrder}>
            {busy ? "جاري تسجيل الطلب…" : "طلب الآن"}
          </button>
          {payReason && <div className="posx-fire-reason">{payReason}</div>}
        </div>
      </aside>
      {cartDrawerOpen && (
        <button
