...
export async function loadFullOrder(db: Knex, accountId: string, orderId: string) {
  ...
  return {
    ...
    customer_phone: order.delivery_phone_snapshot ?? customer?.phone ?? null,
    customer_address: order.delivery_address ?? customer?.address ?? null,
    delivery_zone_name: order.delivery_zone_name_snapshot ?? null,
