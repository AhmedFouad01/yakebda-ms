...
3. **Policy value register (plan §2) arrived with all nine values unfilled.** Per plan §10.1.3, every value was recorded as `PENDING VALUE` at CP0. **Superseded 2026-07-21 (before CP3):** the policy model changed to *Standard Default Policy + Per-Tenant Override* — see the replaced ADR-004. Engine constants (auto settlement at close, close-date recognition, 4dp/2dp half-up) are fixed in the product; VAT/revenue-recognition/operational-day/threshold ship as editable per-tenant defaults; a default chart including rounding account `4090` is seeded so settlement is never blocked for a fresh tenant. Blocking (422) applies only when a tenant has unmapped the rounding account.
